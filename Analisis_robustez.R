...
# OJO: se usan los años que REALMENTE aparecen en W_por_anio, no
# anios_bce_disponibles a ciegas. 2021 se carga pero nunca se usa para
# repartir ventas (dta_empresas_ventas no tiene ANIO FISCAL = 2021), así
# que no genera columna peso_2021 -- comparar contra un año sin datos
# reventaba el pivot_wider. Ver la sección 4b más abajo para una
# comparación que sí incluye 2021, sin depender de ventas.
anios_con_datos <- W_por_anio %>% distinct(anio_bce) %>% pull(anio_bce) %>% sort()
anio_min <- min(anios_con_datos)
anio_max <- max(anios_con_datos)
...
# =====================================================================
# 4b. ESTABILIDAD DE LA ESTRUCTURA "CRUDA" (sin pasar por ventas) -------
# =====================================================================
# La comparación de arriba usa Tab2_full, que reparte según las ventas
# de CADA año -- por eso 2021 queda fuera (no hay ventas ese año). Esta
# versión promedia el w_ij del BCE directo, con peso IGUAL entre los
# códigos CIIU de cada clúster (no ponderado por ventas), así que sí
# funciona para los 4 años, incluido 2021.

codigo_a_cluster <- dta_bce_codigo %>%
  distinct(`CÓDIGO DE INDUSTRIA ECUATORIANO_CIE`, CIIU4_6D) %>%
  left_join(dta_bitacora2026 %>% distinct(codigo_actividad, CLUSTER),
            by = c("CIIU4_6D" = "codigo_actividad")) %>%
  filter(!is.na(CLUSTER)) %>%
  distinct(`CÓDIGO DE INDUSTRIA ECUATORIANO_CIE`, CLUSTER)

estructura_cruda_por_anio <- dta_long_multi %>%
  left_join(codigo_a_cluster %>% rename(CLUSTER_ORIGEN = CLUSTER),
            by = c("Codigo" = "CÓDIGO DE INDUSTRIA ECUATORIANO_CIE")) %>%
  left_join(codigo_a_cluster %>% rename(CLUSTER_DESTINO = CLUSTER),
            by = c("col" = "CÓDIGO DE INDUSTRIA ECUATORIANO_CIE")) %>%
  filter(!is.na(CLUSTER_ORIGEN), !is.na(CLUSTER_DESTINO)) %>%
  group_by(anio_bce, CLUSTER_ORIGEN, CLUSTER_DESTINO) %>%
  summarise(peso_crudo = mean(prop, na.rm = TRUE), .groups = "drop")

estructura_cruda_ancho <- estructura_cruda_por_anio %>%
  pivot_wider(names_from = anio_bce, values_from = peso_crudo,
              values_fill = 0, names_prefix = "peso_")

anio_min_crudo <- min(anios_bce_disponibles)   # aquí sí existen todos, incluido 2021
anio_max_crudo <- max(anios_bce_disponibles)
col_min_crudo  <- paste0("peso_", anio_min_crudo)
col_max_crudo  <- paste0("peso_", anio_max_crudo)

estabilidad_estructura_cruda <- estructura_cruda_ancho %>%
  group_by(CLUSTER_ORIGEN) %>%
  summarise(
    cambio_total = sum(abs(.data[[col_min_crudo]] - .data[[col_max_crudo]]), na.rm = TRUE) / 2,
    .groups = "drop"
  ) %>%
  arrange(desc(cambio_total))

cat("\n=== 4b. ESTABILIDAD DE LA ESTRUCTURA CRUDA (BCE puro)",
    anio_min_crudo, "->", anio_max_crudo, "===\n")
cat("Sin ponderar por ventas -- compara los 4 años, incluido 2021:\n")
print(head(estabilidad_estructura_cruda, 10))
